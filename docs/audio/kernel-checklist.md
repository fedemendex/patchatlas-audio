...
(`web/src/audio/modules/**`, `web/src/audio/engine/**`). It is the enforcement arm of the
...
- [ ] Every tuning, gate, and normalization constant is imported from
      `web/src/audio/engine/units.ts` and conforms to `docs/audio/signals.md`.
...
- [ ] Jack and param names in the `ModuleDSP` entry are seed **names** verified against
      ...
      (`web/src/audio/modules/registry.test.ts`) passes.
...
- [ ] Behavior matches `docs/audio/signals.md` (signal ranges, 1 V/oct pitch, Schmitt gate
